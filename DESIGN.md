...
- The following scripts enable generating the supporting prompt and validator schema automatically for the ticketing agent: 
...
The Llama3.2:latest achieves the least accuracy and should be excluded from evaluation. Its tickets.json was generated by disabling the output format validation during the Ollama call, as the model sometimes fails to follow the output structure.
...
- The knowledge text is very small; most of the content is relevant for ticketing and can fully fit into the LLM context window. But, having a summary of a vetted decision, e.g., stored as SKILL.md and extracted from the AI Design Assistant agent, instead of supplying the entire knowledge.md directly to the ticketing agent prompt, could improve clarity and accuracy, and reduce validation complexity.
